...
placeholder text, and an FPS counter.
...
- At startup, `buildSphereLut()` projects globe pixels to longitude and stores
  longitude/shading in a compact 16-bit PSRAM LUT. Latitude is shared by each
  scanline.
...
rate on the target board is approximately 19–20 FPS, compared with 5 FPS for the
initial single-buffer renderer.

## Capturing the framebuffer

This is available only in `waveshare_amoled_143_debug`. The firmware accepts `S`
over USB serial and returns the current 466×466 RGB565 frame with dimensions and
CRC32. Capture it as a PNG with:
...
and rivers to major features, and bakes detailed/blurred intensity channels.
